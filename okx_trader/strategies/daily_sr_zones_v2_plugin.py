...
def _resolve_daily_sr_zones_v2(inputs: VariantSignalInputs) -> Dict[str, Any]:
    ...
    zone_w = max(close_f * 0.0075, atr_v * 0.9)
    breakout_pad = zone_w * 0.15
    ...
    bull_pin = (
        close_f > curr_o
        and lower_wick >= max(body * 1.2, atr_v * 0.25)
        and lower_wick >= upper_wick * 1.3
        and body / true_range <= 0.65
    )
    bear_pin = (
        close_f < curr_o
        and upper_wick >= max(body * 1.2, atr_v * 0.25)
        and upper_wick >= lower_wick * 1.3
        and body / true_range <= 0.65
    )
    bull_engulf = (
        close_f > curr_o
    ...
    bull_reclaim = (
        close_f > curr_o
        and body >= atr_v * 0.25
        and close_f >= curr_hi - true_range * 0.25
        and lower_wick >= body * 0.8
    )
    bear_reclaim = (
        close_f < curr_o
        and body >= atr_v * 0.25
        and close_f <= curr_lo + true_range * 0.25
        and upper_wick >= body * 0.8
    )
    vol_spike = volume_avg > 0 and volume >= volume_avg * 1.2
    ...
    bounce_long = close_f >= support - zone_w * 0.20 and close_f > curr_o and close_f > ema21 * 0.992
    bounce_short = close_f <= resistance + zone_w * 0.20 and close_f < curr_o and close_f < ema21 * 1.008

    long_reversal = (
        touch_support
        and bounce_long
        and (bull_pin or bull_engulf or bull_reclaim)
        and (rsi_v <= 48.0 or vol_spike)
        and (not weekly_down)
    )
    short_reversal = (
        touch_resistance
        and bounce_short
        and (bear_pin or bear_engulf or bear_reclaim)
        and (rsi_v >= 52.0 or vol_spike)
        and (not weekly_up)
    )

    strong_breakout = body >= atr_v * 0.6 and body / true_range >= 0.50
    breakout_long = (
        close_f >= resistance + breakout_pad
        and close_f > curr_o
        and strong_breakout
        and (vol_spike or close_f >= curr_hi - true_range * 0.20)
        and weekly_up
    )
    breakout_short = (
        ...
        and close_f < curr_o
        and strong_breakout
        and (vol_spike or close_f <= curr_lo + true_range * 0.20)
        and weekly_down
    )
    ...
    bear_reject_from_res = touch_resistance and bounce_short and (bear_pin or bear_engulf or bear_reclaim)
    bull_reject_from_sup = touch_support and bounce_long and (bull_pin or bull_engulf or bull_reclaim)
...
def register(
    *,
    ...
    register_variant_input_resolver("daily_sr_zones_v2", _resolve_daily_sr_zones_v2)
