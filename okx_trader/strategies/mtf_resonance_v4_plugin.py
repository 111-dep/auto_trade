...
def _resolve_mtf_resonance_v4(inputs: VariantSignalInputs) -> Dict[str, Any]:
    p = inputs.p
    ...
    loc_trend_long = (
        ...
        and loc_rsi >= max(50.0, float(getattr(p, "rsi_long_min", 50.0)))
    ...
    hour_zone_touch = hour_low <= zone_hi * (1.0 + 2.0 * pullback_tol) and hour_low >= zone_lo * (1.0 - 3.0 * pullback_tol)
    hour_pullback_long = hour_zone_touch and hour_rsi <= min(50.0, float(getattr(p, "rsi_short_max", 50.0) + 2.0))
    hour_reclaim = hour_close >= zone_lo and (hour_close >= hour_open or hour_close >= hour_prev_close)
    ...
    ltf_rsi_ok = ltf_rsi >= max(50.0, float(getattr(p, "rsi_long_min", 50.0)))
    trigger_long = bullish_candle and above_ltf_ema and not_chasing and micro_break and (macd_turn_up or ltf_rsi_ok)
    ...
    return {
        ...
        "mtf_trigger_score": int(
            ...
            + int(not_chasing)
            + int(micro_break)
            + int(macd_turn_up or ltf_rsi_ok)
        ),
...
def register(
    *,
    ...
    register_variant_input_resolver("mtf_resonance_v4", _resolve_mtf_resonance_v4)
