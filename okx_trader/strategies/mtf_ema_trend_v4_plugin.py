...
def _resolve_mtf_ema_trend_v4(inputs: VariantSignalInputs) -> Dict[str, Any]:
    p = inputs.p
    ...
    pullback_tol = max(0.0008, float(getattr(p, "pullback_tolerance", 0.0015)) * 1.20)
    ...
    hour_zone_touch_long = (
        hour_low <= zone_hi * (1.0 + 2.0 * pullback_tol)
        and hour_low >= zone_lo * (1.0 - 4.0 * pullback_tol)
    )
    hour_reclaim_long = hour_close >= zone_mid and hour_close >= hour_open and hour_close >= hour_prev_close

    hour_zone_touch_short = (
        hour_high >= zone_lo * (1.0 - 2.0 * pullback_tol)
        and hour_high <= zone_hi * (1.0 + 4.0 * pullback_tol)
    )
    ...
    ltf_reclaim_long = (
        ...
        and close_f > ltf_ema
        and (
            prev_close <= ltf_ema * (1.0 + pullback_tol)
            or curr_lo <= ltf_ema * (1.0 + pullback_tol)
            or hour_zone_touch_long
        )
    )
    ltf_reclaim_short = (
        close_f < curr_o
        ...
        and (
            prev_close >= ltf_ema * (1.0 - pullback_tol)
            or curr_hi >= ltf_ema * (1.0 - pullback_tol)
            or hour_zone_touch_short
        )
    )
    ...
    long_entry_l3 = (
        ...
        and hour_rsi <= 58.0
        and macd_hist > 0.0
        and close_f <= prev_hi * (1.0 + max(0.0010, 2.0 * pullback_tol))
    ...
    short_entry_l3 = (
        ...
        and hour_rsi >= 54.0
        and macd_hist < 0.0
        and close_f >= prev_lo * (1.0 - max(0.0010, 2.0 * pullback_tol))
        and bool(inputs.not_chasing_short)
    )

    long_level = 3 if long_entry_l3 else 0
    short_level = 3 if short_entry_l3 else 0
    ...
    return {
        "variant": "mtf_ema_trend_v4",
        "vol_ok": True,
        "fresh_break_long": bool(breakout_long),
        "fresh_break_short": bool(breakout_short),
        "mtf4_htf_up": bool(htf_up),
        "mtf4_htf_down": bool(htf_down),
        "mtf4_loc_trend_long": bool(loc_trend_long),
        "mtf4_loc_trend_short": bool(loc_trend_short),
        "mtf4_touched_long_zone": bool(touched_long_zone),
        "mtf4_touched_short_zone": bool(touched_short_zone),
        "mtf4_hour_reclaim_long": bool(hour_reclaim_long),
        "mtf4_hour_reclaim_short": bool(hour_reclaim_short),
        "mtf4_momentum_long_score": int(momentum_long_score),
        "mtf4_momentum_short_score": int(momentum_short_score),
...
def register(
    *,
    ...
    register_variant_input_resolver("mtf_ema_trend_v4", _resolve_mtf_ema_trend_v4)
